...
ACANFD_SAME_Settings::ACANFD_SAME_Settings (const ClockSelection inClockSelection,
                                                            const uint32_t inDesiredArbitrationBitRate,
                                                            const DataBitRateFactor inDataBitRateFactor,
                                                            const uint32_t inTolerancePPM) :
ACANFD_SAME_Settings (inClockSelection, inDesiredArbitrationBitRate, 75, inDataBitRateFactor, 75, inTolerancePPM) {
}
...
mClockSelection (inClockSelection) {
  const uint32_t dataBitRate = inDesiredArbitrationBitRate * uint32_t (inDataBitRateFactor) ;
  uint32_t dataTQCount = std::min (MAX_DATA_TQ_COUNT, MAX_ARBITRATION_TQ_COUNT / uint32_t (inDataBitRateFactor)) ;
  ...
  const uint32_t FDCAN_CLOCK_FREQUENCY = FDCANClockFrequency () ;
...
uint32_t ACANFD_SAME_Settings::FDCANClockFrequency (void) const {
  uint32_t result = 48 * 1000 * 1000 ; // 48 MHz
  ...
  case CPU_CLOCK :
    ...
    break ;
  case CPU_CLOCK_DIVIDED_BY_2 :
    result = CPUClockFrequency () / 2 ;
    break ;
  case CPU_CLOCK_DIVIDED_BY_3 :
    result = CPUClockFrequency () / 3 ;
    break ;
  case CPU_CLOCK_DIVIDED_BY_4 :
    result = CPUClockFrequency () / 4 ;
    break ;
  case CPU_CLOCK_DIVIDED_BY_5 :
    result = CPUClockFrequency () / 5 ;
    break ;
  case CPU_CLOCK_DIVIDED_BY_6 :
    result = CPUClockFrequency () / 6 ;
    break ;
  case DPLL1_CLOCK :
    result = DPPL1ClockFrequency () ;
    break ;
  case DPLL1_CLOCK_DIVIDED_BY_2 :
    result = DPPL1ClockFrequency () / 2 ;
    break ;
  case DPLL1_CLOCK_DIVIDED_BY_3 :
    result = DPPL1ClockFrequency () / 3 ;
    break ;
  case DPLL1_CLOCK_DIVIDED_BY_4 :
    result = DPPL1ClockFrequency () / 4 ;
    break ;
  }
  return result ;
}
...
uint32_t ACANFD_SAME_Settings::actualArbitrationBitRate (void) const {
  const uint32_t FDCAN_CLOCK_FREQUENCY = FDCANClockFrequency () ;
  const uint32_t TQCount = 1 /* Sync Seg */ + mArbitrationPhaseSegment1 + mArbitrationPhaseSegment2 ;
  return FDCAN_CLOCK_FREQUENCY / (mBitRatePrescaler * TQCount) ;
}

//----------------------------------------------------------------------------------------

uint32_t ACANFD_SAME_Settings::actualDataBitRate (void) const {
  const uint32_t FDCAN_CLOCK_FREQUENCY = FDCANClockFrequency () ;
...
bool ACANFD_SAME_Settings::exactArbitrationBitRate (void) const {
  const uint32_t FDCAN_CLOCK_FREQUENCY = FDCANClockFrequency () ;
  const uint32_t TQCount = 1 /* Sync Seg */ + mArbitrationPhaseSegment1 + mArbitrationPhaseSegment2 ;
  return FDCAN_CLOCK_FREQUENCY == (mBitRatePrescaler * mDesiredArbitrationBitRate * TQCount) ;
}

//----------------------------------------------------------------------------------------

bool ACANFD_SAME_Settings::exactDataBitRate (void) const {
  const uint32_t FDCAN_CLOCK_FREQUENCY = FDCANClockFrequency () ;
  const uint32_t TQCount = 1 /* Sync Seg */ + mDataPhaseSegment1 + mDataPhaseSegment2 ;
  return FDCAN_CLOCK_FREQUENCY == (mBitRatePrescaler * mDesiredArbitrationBitRate * TQCount * uint32_t (mDataBitRateFactor)) ;
}

//----------------------------------------------------------------------------------------

uint32_t ACANFD_SAME_Settings::ppmFromWishedBitRate (void) const {
  const uint32_t FDCAN_CLOCK_FREQUENCY = FDCANClockFrequency () ;
  const uint32_t TQCount = 1 /* Sync Seg */ + mArbitrationPhaseSegment1 + mArbitrationPhaseSegment2 ;
  const uint32_t W = TQCount * mDesiredArbitrationBitRate * mBitRatePrescaler ;
  const uint64_t diff = (FDCAN_CLOCK_FREQUENCY > W)
    ? (FDCAN_CLOCK_FREQUENCY - W)
    : (W - FDCAN_CLOCK_FREQUENCY)
  ;
  const uint64_t ppm = uint64_t (1000 * 1000) ;
...
uint32_t ACANFD_SAME_Settings::arbitrationSamplePointFromBitStart (void) const {
  const uint32_t TQCount = 1 /* Sync Seg */ + mArbitrationPhaseSegment1 + mArbitrationPhaseSegment2 ;
...
uint32_t ACANFD_SAME_Settings::dataSamplePointFromBitStart (void) const {
  const uint32_t TQCount = 1 /* Sync Seg */ + mDataPhaseSegment1 + mDataPhaseSegment2 ;
...
uint32_t ACANFD_SAME_Settings::CANFDBitSettingConsistency (void) const {
