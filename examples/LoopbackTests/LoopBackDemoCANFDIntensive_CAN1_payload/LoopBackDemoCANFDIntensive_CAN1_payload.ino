...
void setup() {
  ...
  settings.mHardwareTransmitBufferPayload = ACANFD_SAME_Settings::PAYLOAD_32_BYTES;
  //--- Limit hardware receive FIFO 0 to 24 data bytes
  settings.mHardwareRxFIFO0Payload = ACANFD_SAME_Settings::PAYLOAD_24_BYTES;
...
void loop() {
  ...
  if (gOk && can1.receiveFD0(receivedFrame)) {
    ...
    if (storedFrame.type != CANFDMessage::CAN_REMOTE) {
      ...
      const uint32_t lg = ACANFD_SAME_Settings::frameDataByteCountForPayload(minPayload);
